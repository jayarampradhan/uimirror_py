...
def buildLoginGetArguments(destination = None, tab = None, sub_tab = None, itest = None):
        '''
           ...
           @param forgotPasswordTab: Forgot password tab identifier.
           @param forgotPasswordSubTab: Sub Tab of forgot password tab.
        ...
        _DES_ARG = 'des=%s';
        _FORGOT_PWD_TAB = 'fgp=%s'
        _FORGOT_PWD_SUB_TAB = 'fgpm=%s'
        _ISSUE_ARG = 'itest=%s'
        
        if tab:
            _rs_uri += (_FORGOT_PWD_TAB %tab);
            apply_sep = True;
        if sub_tab:
            if apply_sep:
                _rs_uri += (_GET_SEPRATOR + _FORGOT_PWD_SUB_TAB %sub_tab);
            else:
                _rs_uri += (_FORGOT_PWD_SUB_TAB %sub_tab);
                apply_sep = True;
        
        if itest:
            if apply_sep:
                _rs_uri += (_GET_SEPRATOR + _ISSUE_ARG %itest);
            else:
