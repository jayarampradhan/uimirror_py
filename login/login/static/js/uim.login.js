...
var UIM = (function(LOGIN) {
	...
	function isValidateLoginForm(){
		...
		if(!_uid && !_pwd){
			_valid_flag = false;
			updateLoginErrMsg('Key In User Id And Password.');
			_uid_elm.addClass('uimFieldErr');
			_pwd_elm.addClass('uimFieldErr');
		}else if(!_pwd){
			_valid_flag = false;
			updateLoginErrMsg('Key In Password.');
			_pwd_elm.addClass('uimFieldErr');
		}else if(!_uid){
			_valid_flag = false;
			updateLoginErrMsg('Key In User Id.');
			_uid_elm.addClass('uimFieldErr');
		}
		return _valid_flag;
	}
	function isValidForgotPassWordForm(){
		var _option_val = $('input[name=preoption]:checked', '#uimForgotPasswordForm').val(),
			_valid_flag = true;
		console.log("Options Value",_option_val);
		if(_option_val === '1'){
			var _uimEmail_elm = $('#uimSignedUpEmail').removeClass('uimFieldErr');
			if(!_uimEmail_elm.val()){
				_valid_flag = false;
				updateForgotPwdErrMsg('Key In User Id And Password.');
			...
			if(!_uimEmail_val && !_uimrRecent_pwd_val && !_uimrAlternative_em_val){
				updateForgotPwdErrMsg('Key In All the fields.');
				_uimEmail_elm.addClass('uimFieldErr');
				...
				updateForgotPwdErrMsg('Key In Registered Email and Recent Password.');
				_uimEmail_elm.addClass('uimFieldErr');
				_uimrRecent_pwd_elm.addClass('uimFieldErr');
				_valid_flag = false;
			}else if(!_uimrRecent_pwd_val && !_uimrAlternative_em_val){
				updateForgotPwdErrMsg('Key In Recent Password and Alternative Email Id.');
				_uimrRecent_pwd_elm.addClass('uimFieldErr');
				_uimrAlternative_em_elm.addClass('uimFieldErr');
				_valid_flag = false;
			}else if(!_uimEmail_val && !_uimrAlternative_em_val){
				updateForgotPwdErrMsg('Key In Regsitered Email and Alternative Email Id.');
				_uimEmail_elm.addClass('uimFieldErr');
				_uimrAlternative_em_elm.addClass('uimFieldErr');
				_valid_flag = false;
			}else if(!_uimEmail_val){
				updateForgotPwdErrMsg('Key In Registered Email.');
				_uimEmail_elm.addClass('uimFieldErr');
				_valid_flag = false;
			}else if(!_uimrRecent_pwd_val){
				updateForgotPwdErrMsg('Key In Most Recent Password.');
				_uimrRecent_pwd_elm.addClass('uimFieldErr');
				_valid_flag = false;
			}else if(!_uimrAlternative_em_val){
				updateForgotPwdErrMsg('Key In Alternative Email Id.');
				_uimrAlternative_em_elm.addClass('uimFieldErr');
				_valid_flag = false;
			}
		}
